...
function who_hit_the_page( $page ) {
	...
	*
	* Restrict WordPress IP from being counted - done
	* Allow to reset counters for specific page - done
	* Reset All counters - done
	*
	* Collapse or expand all IP details
	* Geolocate IP address
	* User IP address and all browsers used
	*
	* Count Total Hits per IP - done
	*
...
function whtp_reset_page_count( $page = ""){
	...
	if( $page != "all" ){
		...
		if ( $update_page ){
			echo '<div class="success-msg">The count for the page "' .$page . 
			'" has been reset successfully.</div>'; 
		}else{
			echo '<div class="error-msg">Failed to reset count for page "'. $page . '"' .
			': ' . mysql_error() . '</div>';
		}
	}else{
		#reset all
...
function whtp_delete_ip (){
	...
	if ( $delete_ip == "this_ip" ) {
		...
		if ( $del ){
			echo '<div class="success-msg">The IP address has been removed from the database</div>';
		}else{
...
function whtp_hit_info(){
	...
	# the script or file requested
	$file_op = $_SERVER['SCRIPT_NAME'];
	*/
